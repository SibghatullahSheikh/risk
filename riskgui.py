...
def drawterritory(terr, shaded):
    """Draw an entire territory (possibly shaded)"""
    terr = territories[terr.name]
    if shaded:
...
class PlayerDialog(tkSimpleDialog.Dialog):
    ...
    def body(self, master):
        ...
        self.pname.pack()
        self.set_ai = Tkinter.Button(self, text="Set AI",  command=self.setai)
        self.set_ai.pack()
        self.ai_filename = ""
        return self.pname
    ...
    def setai(self):
        ...
        if self.pstr.get() == "":
            name, _ = splitext(basename(self.ai_filename))
            self.pstr.set(name)
...
def setupdata():
    ...
    playerMenu.add_command(label="Fortify", command=riskengine.startfortifying)
