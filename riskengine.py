...
def attack(territory1, territory2, d_attack=-1, d_defence=-1):
    """Have one territory attack another"""
    if d_attack == -1:
        d_attack = min(territory1.armies - 1, 3)
    if d_defence == -1:
        d_defence = min(territory2.armies, 3)
    
    if d_attack < 1 or d_attack >= territory1.armies or d_attack > 3:
        return
    if d_defence < 1 or d_defence > territory2.armies or d_defence > 3:
        return
    if not [x for x in territory1.neighbors if x == territory2]:
        return
    numdice = min(d_attack, d_defence)
    
    dice1 = [random.randint(1, 6) for _ in range(d_attack)]
    dice1.sort()
    dice1.reverse()
    
    dice2 = [random.randint(1, 6) for _ in range(d_defence)]
    dice2.sort()
    dice2.reverse()
    ...
    if territory2.armies == 0:
        ...
        changecontrol(territory1, territory2, d_attack)
...
def createcards():
    ...
    for terr in range(2, len(territories)):
        car = Card(territories.values()[terr].name, random.choice(CardSet.PICTURES))
        allcards.append(car)
...
class CardSet:
    PICTURES = ["Cannon", "Horse", "Soldier"]
    
    def __init__(self, cards):
        assert len(cards) == 3
        self.pictures = [c.picture for c in cards]
        self.pictures.sort()
    
    def tris(self, picture):
        for p in self.pictures:
            if p != picture: return False
        return True
    
    def all_different(self):
        for c in CardSet.PICTURES:
            if c not in self.pictures:
                return False
        return True
    
    def value(self):
        for picture, value in (("Cannon", 4), ("Soldier", 6), ("Horse", 8)):
            if self.tris(picture):
                return value
        
        if self.all_different():
            return 10
        
        if (self.pictures[0] == self.pictures[1] and
            self.pictures[2] == "Wildcard"):
            return 12
